...
dat1<- lapply(dat, function(x) { 
        ...
        x$seq<-s[-1]  ;return(x)
        })                # mes<-dat[[5]]   mes[,1]     names(dat1)    #   x<-dat[[3]]  x2<-dat1[[5]] names(x2) names(dat1[[3]])
...
# x<-dat_filt[[11]]     filter_at(vars(b, c), any_vars(complete.cases(.))   any_vars(!is.na(.))
