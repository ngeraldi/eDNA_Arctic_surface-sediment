...
mod_all<- mvabund::manyany("gam", cc,  y~ area_cat_3 + s(land_dist_scaled,  m=wig) + s(pp_mean_scaled,  m=wig)+ s(SST_mean_scaled,  m=wig) , data=var, family="nb" ) 
...
mod<- mvabund::manyglm(mv ~ (bot_temp_mean_scaled   + SST_change_scaled +  par_scaled + pp_mean_scaled + water_depth_scaled), data=var, family="negative.binomial", composition= "FALSE", nboot=1000, cor.type="shrink",show.cor=TRUE, nCore=3) # 
    # plot(mod)
    #  boxplot(cc, yaxis="Taxa")
     # mod_sum <- anova(mod)
...
mod_anov<-anova(mod, resamp="pit.trap",nBoot=1000,cor.type="shrink",p.uni="adjusted")##
#   binomial("cloglog")


mod_sum<-summary(mod, resamp="pit.trap",nboot=1000,cor.type="shrink",show.cor=TRUE, nCore=3)
...
num_clust<-8 # number of clusters for taxa
# base on t.cn
x_labels<-c( "Bottom temp.", "PAR", "Primary Prod.","SST change" ,"Water depth") # should be alphabetical, check match order of mult_dat3
# set p value minimum fo individual taxa to include in plot
p_min_val<- 0.2
# 
inner_margins<-c(.3,27,1,1)  # increase left - second number for larger taxa names - oppossite too
...
ggsave(mult_plot, file=paste(dir,plot_file,"/multvar_taxa_abund.tiff",sep=""), width = 20, height = 25, units = "cm", dpi="print")
...
example_NMDS <- metaMDS( log(cc+1),    k=5 , trymax=40) # Our community-by-species matrix
...
pdf(paste0(dir,plot_file,"/arctic_surf_mds_GAMs.pdf" ), width = 10, height = 15 )
...
# sites by numbers
pl<- ordiplot(example_NMDS,type="n", xlim=c(-1.5,1.5) , ylim=c(-1.5,1.5))
...
# label species
ordiplot(example_NMDS,type="n", xlim=c(-1.5,1.5) , ylim=c(-1.5,1.5))
points(pl, "sites", pch=1, col=col_y_lab[2], cex= 0.7)
