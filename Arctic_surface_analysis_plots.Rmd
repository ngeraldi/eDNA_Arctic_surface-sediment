...
```{r worms_info, eval=FALSE, include=FALSE}
...
y_labels_make<- taxa_file %>%   # names(taxa_file)    ylabels$lineage_stat
  ...
  mutate(lineage_stat= gsub("-", ".", lineage_stat)) %>% 
  inner_join(rn) %>% 
  unite(y_labels, Phylum:Species , sep=";", remove=FALSE) %>% #. change here for y labels!!!!
  mutate(y_labels=gsub("NA", "Unclassified", y_labels))
 
y_labels <- clustorder %>% 
    rename(lineage_stat=lineage) %>% 
    left_join(y_labels_make) %>% 
    mutate(y_labels=gsub("brevicornis brevicornis", "brevicornis", y_labels))
